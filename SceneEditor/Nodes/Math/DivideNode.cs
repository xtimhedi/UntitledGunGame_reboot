...
{
    public partial class DivideNode : CustomGraphNode
    {
        public override void _Ready()
        {
            NodeType = NodeTypeEnum.Pure;
            Title = "Divide Values (Pure)";
        ...
        {
            int a = InputValues.TryGetValue(0, out var valA) && valA is int intA ? intA : 1;
            int b = InputValues.TryGetValue(1, out var valB) && valB is int intB ? intB : 1;
            int result = 0;
            if (a != 0 && b != 0)
            {
                result = a / b;
